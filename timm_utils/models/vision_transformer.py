...
class Attention_croma(nn.Module):
    ...
    def forward(self, x, alibi):
        ...
        x = self.proj(rearrange(out, 'b h n d -> b n (h d)'))
        x = self.proj_drop(x)
        return x
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.attn = Attention_croma(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
    ...
    def forward(self, x, alibi):
        x = x + self.drop_path(self.attn(self.norm1(x), alibi))
